...
$getCount = mysqli_result(mysqli_query($conn, "SELECT COUNT(id) FROM servers"), 0); // Взимаме колко броя новини има
...
$pagination = pagination($getCount, [
    ...
    'per_page' => 5, // по колко резултата да се показват на страница
...
if ($cat) {
    $sql = "SELECT * FROM servers WHERE type = {$cat} ORDER by vip DESC LIMIT {$pagination['limit']['first']}, {$pagination['limit']['second']}";
    $nums = "SELECT COUNT(*) as numservers, SUM(players) as numplayers, SUM(maxplayers) as slots FROM servers WHERE type= {$cat}";
} else {
    $sql = "SELECT * FROM servers ORDER BY vip DESC LIMIT {$pagination['limit']['first']}, {$pagination['limit']['second']}";
...
?>
    ...
    <div class="container mt-5">
        ...
        <?php if ($getServ->num_rows > 0) { ?>
            ...
            <?php echo $pagination['output']; // Изкарваме страницирането?>
        <?php } else { ?>
